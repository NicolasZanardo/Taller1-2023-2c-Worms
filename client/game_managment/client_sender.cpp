    ...
    switch (event) {
        case GameEvent::MOVE_LEFT_INIT:
            return std::make_shared<NetMessageGameAction>(
                this->client_id, ActionTypeDto::moving_left_init
            );
        case GameEvent::MOVE_LEFT_END:
            return std::make_shared<NetMessageGameAction>(
                this->client_id, ActionTypeDto::stop_moving
            );
        case GameEvent::MOVE_RIGHT_INIT:
            return std::make_shared<NetMessageGameAction>(
                this->client_id, ActionTypeDto::moving_right_init
            );
        ...
        case GameEvent::JUMP_FORWARD:
            return std::make_shared<NetMessageGameAction>(
                this->client_id, ActionTypeDto::jump_forward
            );
        case GameEvent::JUMP_BACKWARDS:
            return std::make_shared<NetMessageGameAction>(
                this->client_id, ActionTypeDto::jump_back
            );
    }
    return std::make_shared<NetMessageGameAction>(
        this->client_id, ActionTypeDto::stop_moving
